...
const genererGrilleAleatoire = () => {
  const grille = [];
  for (let i = 0; i < nombreLignes; i++) {
    grille[i] = Array.from(Array(nombreColonnes), () => (Math.random() > 0.7 ? 1 : 0));
  }
  return grille;
};
...
const JeuDeLaVie = () => {
  ...
  const etapeSuivante = () => {
    ...
    for (let i = 0; i < nombreLignes; i++) {
      for (let j = 0; j < nombreColonnes; j++) {
        ...
        const nombreVoisinsVivants = voisins.reduce((acc, cellule) => acc + cellule, 0);
  ...
  return (
    <div className='py-3 px-10 md:w-full W-fit gap-x-5 md:h-screen h-fit bg-yellow-500 flex flex-row items-center'>
      ...
      <div className='w-[300px] px-5 py-3 h-[500px] flex flex-col flex-wrap justify-around rounded-xl items-center bg-black' style={{ marginBottom: '10px' }}>
        <h1 className='texte text-2xl uppercase text-white'>Jeu de la vie</h1>
        <button className='bg-sky-600 min-w-[200px] min-h-[50px] text-white px-4 py-1 text-xl rounded-lg' onClick={demarrerJeu}>Démarrer</button>
        <button className='bg-red-600 min-w-[200px] min-h-[50px] text-white px-4 py-1 text-xl rounded-lg' onClick={arreterJeu}>Arrêter</button>
        <button className='bg-sky-600 min-w-[200px] min-h-[50px] text-white px-4 py-1 text-xl rounded-lg' onClick={etapeSuivante}>Suivant</button>
        <button className='bg-gray-600 min-w-[200px] min-h-[50px] text-white px-4 py-1 text-xl rounded-lg' onClick={reinitialiserJeu}>Réinitialiser</button>
      </div>
    </div>
  );
};
